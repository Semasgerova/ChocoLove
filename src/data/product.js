const data = {
  homeShop: [
    {
      id: 1,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-03-600x600.png311",
      title: "Truffle With Vanilla",
      price: 25
    },
    {
      id: 2,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-06-600x600.png",
      title: "Milk Chocolate With Peanuts",
      price: 42.49
    },
    {
      id: 3,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-04-600x600.png",
      title: "Pastry Chef Fantasy",
      price: 13.49
    },
    {
      id: 4,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-05-600x600.png",
      title: "Almond Dark Chocolate",
      price: 12.99
    }
  ],
  homeShopAz: [
    {
      id: 1,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-03-600x600.png",
      title: "Vanilli Truffle",
      price: 25
    },
    {
      id: 2,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-06-600x600.png",
      title: "Fıstıqlı Südlü Şokolad",
      price: 42.49,
      category: "Südlü"
    },
    {
      id: 3,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-04-600x600.png",
      title: "Pasta şefi fantezisi",
      price: 13.49
    },
    {
      id: 4,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-05-600x600.png",
      title: "Badamlı Tünd Şokolad",
      price: 12.99
    }
  ],
  homeGallery: [
    {
      id: 5,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/gallery-01-e1597251883779-360x360.jpg"
    },
    {
      id: 6,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/gallery-02-480x480.jpg"
    },
    {
      id: 7,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/gallery-03-480x480.jpg"
    },
    {
      id: 8,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/gallery-04-480x480.jpg"
    }
  ],
  products: [
    {
      id: 9,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-01-600x600.png",
      title: "Chocolate Covered Roast",
      price: 17.99,
      category: "Chocolate",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 10,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-02-600x600.png",
      title: "Honey Roasted Nuts",
      price: 19.99,
      category: "Deserts",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 11,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-03-600x600.png",
      title: "Truffle With Vanilla",
      price: 25,
      category: "Truffles",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 12,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-04-600x600.png",
      title: "Pastry Chef Fantasy",
      price: 13.49,
      category: "Belgian Waffles",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 13,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-05-600x600.png",
      title: "Almond Dark Chocolate",
      price: 12.99,
      category: "Dark Chocolate",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 14,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-07-600x600.png",
      title: "Fondant-Fruit Sweets",
      price: 3.49,
      category: "Deserts",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 15,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-06-600x600.png",
      title: "Milk Chocolate With Peanuts",
      price: 42.49,
      category: "Chocolate",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 16,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-12-600x600.png",
      title: "Fruit Praline",
      price: 3.49,
      category: "Deserts",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 17,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-11-600x600.png",
      title: "Drunken Cherry Chocolate",
      price: 42.49,
      category: "Chocolate",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 18,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-08-600x600.png",
      title: "Peppered Dark Chocolate",
      price: 25,
      category: "Dark Chocolate",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 19,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-10-600x600.png",
      title: "Dark Chocolate With Souffle",
      price: 11.49,
      category: "Dark Chocolate",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 20,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-09-600x600.png",
      title: "Chocolate Coffee Candy",
      price: 13.49,
      category: "Chocolate",
      description: "The warmth and bitterness of coffee paired with the sweet smoothness of chocolate…mmmmm. Coffee enhances the flavors of chocolate. Both coffee and chocolate have a variety of unique flavors and are independently complex. When paired together, delicious combinations are created.Dark chocolate is a form of chocolate containing cocoa solids and cocoa butter without the milk or butter found in milk chocolate. Dark chocolate without added sweetener is known as bitter chocolate or unsweetened chocolate."
    },
    {
      id: 21,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/16-1000x800.png",
      title: "Single Layer Chocolate Cake",
      price: 20.49,
      category: "Chocolate",
      description: "This Easy Single Layer Chocolate Cake is utterly moist and topped with melt-in-your-mouth Chocolate Frosting! The perfect dessert for date night in, small birthday celebrations. And no electric mixer required!"
    },
    {
      id: 22,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/13-1000x800.png",
      title: "Chocolate Donuts With Sprinkles",
      price: 49.49,
      category: "Deserts",
      description: "Some days I crave a rich, chocolatey dessert that is both comforting and quick, and these chocolate donuts with sprinkles are the perfect remedy. Chocolate cake donuts are iced with a delicious chocolate glaze and topped with your favorite sprinkles for the perfect combination of indulgence and nostalgia."
    },
    {
      id: 23,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/6-1000x800.png",
      title: "Almond Truffle",
      price: 25.49,
      category: "Truffles",
      description: "Love almond desserts? These Almond Bars are a reader favorite as well as a family favorite treat! Almond truffles have a double dose of almond flavor. The ganache is flavored with almond paste, and the finished truffles are rolled in a coating of crushed, toasted almonds. "
    },
    {
      id: 24,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/19-1000x800.png",
      title: "Chocolate Truffle Ganache",
      price: 45.99,
      category: "Truffles",
      description: "Ganache is mixture of chocolate and cream, used to make truffles and other chocolate candies, or as a filling and topping for cakes and pastries."
    },
    {
      id: 25,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/17-1000x800.png",
      title: "Chocolate Truffle",
      price: 20.49,
      category: "Truffles",
      description: "A chocolate truffle is a type of chocolate confectionery, traditionally made with a chocolate ganache centre coated in chocolate, cocoa powder, coconut, or chopped and toasted nuts (typically hazelnuts or almonds), usually in a spherical, conical, or curved shape."
    },
    {
      id: 26,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/18-1000x800.png",
      title: "Chocolate Truffle",
      price: 35.99,
      category: "Truffles",
      description: "A chocolate truffle is a type of chocolate confectionery, traditionally made with a chocolate ganache centre coated in chocolate, cocoa powder, coconut, or chopped and toasted nuts (typically hazelnuts or almonds), usually in a spherical, conical, or curved shape."
    },
    {
      id: 27,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/9-1000x800.png",
      title: "Chocolate Flavored Cup Cake",
      price: 47.89,
      category: "Belgian Waffles",
      description: "Belgian waffles are usually thicker than their American counterparts. These Belgian beauties are made with a recipe that includes yeasted batter and crunchy pearl sugar. Belgian waffles have extra-deep pockets—the better for filling with butter, jam, maple syrup, or whipped cream. "
    },
    {
      id: 28,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/8-1000x800.png",
      title: "Single Layer Chocolate Cake",
      price: 40.59,
      category: "Chocolate",
      description: "chocolate, food product made from cocoa beans, consumed as candy and used to make beverages and to flavour or coat various confections and bakery products. Rich in carbohydrates, it has several health benefits and is an excellent source of quick energy"
    },
    {
      id: 29,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/10-1000x800.png",
      title: "Chocolate Chips Dessert Cheese Ball",
      price: 20.99,
      category: "Dessert",
      description: "Dessert is a course that concludes a meal. The course consists of sweet foods, such as candy, and possibly a beverage such as dessert wine and liqueur. In some parts of the world, such as Greece and West Africa, and most parts of China, there is no tradition of a dessert course to conclude a meal."
    },
    {
      id: 30,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/11-1000x800.png",
      title: "Chocolate Chips Dessert",
      price: 23.49,
      category: "Dessert",
      description: "Dessert is a course that concludes a meal. The course consists of sweet foods, such as candy, and possibly a beverage such as dessert wine and liqueur. In some parts of the world, such as Greece and West Africa, and most parts of China, there is no tradition of a dessert course to conclude a meal."
    },
    {
      id: 31,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/14-1000x800.png",
      title: "Stroopwafels",
      price: 33.99,
      category: "Belgian Waffles",
      description: "Belgian waffles are usually thicker than their American counterparts. These Belgian beauties are made with a recipe that includes yeasted batter and crunchy pearl sugar. Belgian waffles have extra-deep pockets—the better for filling with butter, jam, maple syrup, or whipped cream. "
    }
  ],
  productsAz: [
    {
      id: 9,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-01-600x600.png",
      title: "Şokoladlı qovrulmuş",
      price: 17.99,
      category: "Şokolad",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 10,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-02-600x600.png",
      title: "Bal Qovrulmuş Fındıq",
      price: 19.99,
      category: "Desertlər",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 11,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-03-600x600.png",
      title: "Vanilli Truffle",
      price: 25,
      category: "Trufflelər",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 12,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-04-600x600.png",
      title: "Xəmir aşpazı fantaziyası",
      price: 13.49,
      category: "Belçika vafliləri",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 13,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-05-600x600.png",
      title: "Badam Tünd Şokolad",
      price: 12.99,
      category: "Tünd şokolad",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 14,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-07-600x600.png",
      title: "Fondant-Meyvə Şirniyyatları",
      price: 3.49,
      category: "Desertlər",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 15,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-06-600x600.png",
      title: "Fıstıqlı Südlü Şokolad",
      price: 42.49,
      category: "Şokolad",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 16,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-12-600x600.png",
      title: "Pralin meyvəsi",
      price: 3.49,
      category: "Desertlər",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 17,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-11-600x600.png",
      title: "Sərxoş Albalı Şokolad",
      price: 42.49,
      category: "Şokolad",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 18,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-08-600x600.png",
      title: "Bibərli Tünd Şokolad",
      price: 25,
      category: "Tünd şokolad",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 19,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-10-600x600.png",
      title: "Sufle ilə Tünd Şokolad",
      price: 11.49,
      category: "Tünd şokolad",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 20,
      image: "http://crems.like-themes.com/wp-content/uploads/2019/06/shop-09-600x600.png",
      title: "Şokoladlı qəhvə konfeti",
      price: 13.49,
      category: "Şokolad",
      description: "Qəhvənin istisi və acılığı şokoladın şirin hamarlığı ilə birləşir... mmmmm. Qəhvə şokoladın dadını artırır. Həm qəhvə, həm də şokolad müxtəlif unikal ləzzətlərə malikdir və müstəqil olaraq mürəkkəbdir. Birlikdə birləşdirildikdə ləzzətli kombinasiyalar yaranır. Tünd şokolad, südlü şokoladda olan süd və ya kərə yağı olmayan kakao qatıları və kakao yağı olan şokolad formasıdır. Əlavə dadlandırıcı olmayan tünd şokolad acı şokolad və ya şəkərsiz şokolad kimi tanınır."
    },
    {
      id: 21,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/16-1000x800.png",
      title: "Bir qat şokoladlı tort",
      price: 20.49,
      category: "Şokolad",
      description: "Bu asan bir qatlı şokoladlı tort tamamilə nəmdir və ağzınızda əriyən Şokoladlı Frosting ilə doldurulur! Tarix gecəsi, kiçik ad günü qeyd etmələri üçün mükəmməl desert. Və heç bir elektrik qarışdırıcı tələb olunmur!"
    },
    {
      id: 22,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/13-1000x800.png",
      title: "Çipləri ilə şokoladlı donuts",
      price: 49.49,
      category: "Desertlər",
      description: "Bəzi günlər mən həm təsəlliverici, həm də tez olan zəngin, şokoladlı desertə can atıram və bu şokoladlı donuts səpilməsi ilə mükəmməl bir vasitədir. Şokoladlı tort donutları ləzzətli şokolad şirəsi ilə buzlanır və zövq və nostaljinin mükəmməl birləşməsi üçün ən sevdiyiniz çiləmələrlə doldurulur."
    },
    {
      id: 23,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/6-1000x800.png",
      title: "Badam Truffle",
      price: 25.49,
      category: "Trufflelər",
      description: "Badamlı desertləri sevirsiniz? Bu Badam Barları həm oxucuların sevimlisi, həm də ailənin sevimli yeməyidir! Badam truffles ikiqat dozada badam ləzzətinə malikdir. Qanaş badam pastası ilə ətirlənir və hazır truffle əzilmiş, qızardılmış badam örtüyündə yuvarlanır."
    },
    {
      id: 24,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/19-1000x800.png",
      title: "Şokoladlı Truffle Qanache",
      price: 45.99,
      category: "Trufflelər",
      description: "Qanache şokolad və kremin qarışığıdır, yerdombalanı və digər şokolad konfetləri hazırlamaq üçün və ya tortlar və xəmirlər üçün doldurma və doldurma kimi istifadə olunur."
    },
    {
      id: 25,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/17-1000x800.png",
      title: "Şokoladlı Truffle",
      price: 20.49,
      category: "Trufflelər",
      description: "Şokoladlı truffle ənənəvi olaraq şokolad, kakao tozu, hindistan cevizi və ya doğranmış və qızardılmış qoz-fındıq (adətən fındıq və ya badam) ilə örtülmüş şokoladlı ganache mərkəzi ilə, adətən sferik, konusvari və ya əyri formada hazırlanmış bir şokolad qənnadı növüdür."
    },
    {
      id: 26,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/18-1000x800.png",
      title: "Şokoladlı Truffle",
      price: 35.99,
      category: "Trufflelər",
      description: "Şokoladlı truffle ənənəvi olaraq şokolad, kakao tozu, hindistan cevizi və ya doğranmış və qızardılmış qoz-fındıq (adətən fındıq və ya badam) ilə örtülmüş şokoladlı ganache mərkəzi ilə, adətən sferik, konusvari və ya əyri formada hazırlanmış bir şokolad qənnadı növüdür."
    },
    {
      id: 27,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/9-1000x800.png",
      title: "Şokolad Ətirli Kubok Tortu",
      price: 47.89,
      category: "Belçika vafliləri",
      description: "Belçika vafliləri adətən amerikalı həmkarlarından daha qalın olur. Bu Belçika gözəllikləri mayalanmış xəmir və xırtıldayan mirvari şəkərini ehtiva edən reseptlə hazırlanır. Belçika vaflilərinin çox dərin cibləri var - kərə yağı, mürəbbə, ağcaqayın siropu və ya çırpılmış kremlə doldurmaq üçün daha yaxşıdır."
    },
    {
      id: 28,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/8-1000x800.png",
      title: "Bir qat şokoladlı tort",
      price: 40.59,
      category: "Şokolad",
      description: "şokolad, kakao paxlasından hazırlanmış, konfet kimi istehlak edilən və içkilərin hazırlanmasında, müxtəlif qənnadı məmulatlarının və çörək məmulatlarının ləzzətləndirilməsi və ya örtülməsi üçün istifadə edilən qida məhsuludur. Karbohidratlarla zəngindir, bu, bir sıra sağlamlıq faydalarına malikdir və əla sürətli enerji mənbəyidir"
    },
    {
      id: 29,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/10-1000x800.png",
      title: "Şokolad çipsli desert pendir topu",
      price: 20.99,
      category: "Desertlər",
      description: "Desert yeməyi yekunlaşdıran bir kursdur. Kurs şirniyyat kimi şirin qidalardan və bəlkə də desert şərabı və likör kimi bir içkidən ibarətdir. Dünyanın bəzi yerlərində, məsələn, Yunanıstan və Qərbi Afrikada və Çinin əksər bölgələrində yeməyi yekunlaşdırmaq üçün desert kursu ənənəsi yoxdur."
    },
    {
      id: 30,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/11-1000x800.png",
      title: "Şokolad çipsli desert",
      price: 23.49,
      category: "Desertlər",
      description: "Desert yeməyi yekunlaşdıran bir kursdur. Kurs şirniyyat kimi şirin qidalardan və bəlkə də desert şərabı və likör kimi bir içkidən ibarətdir. Dünyanın bəzi yerlərində, məsələn, Yunanıstan və Qərbi Afrikada və Çinin əksər bölgələrində yeməyi yekunlaşdırmaq üçün desert kursu ənənəsi yoxdur."
    },
    {
      id: 31,
      image: "https://opencart.workdo.io/chocolate/image/cache/catalog/product/14-1000x800.png",
      title: "Stroopwafel'lər",
      price: 33.99,
      category: "Belçika vafliləri",
      description: "Belçika vafliləri adətən amerikalı həmkarlarından daha qalın olur. Bu Belçika gözəllikləri mayalanmış xəmir və xırtıldayan mirvari şəkərini ehtiva edən reseptlə hazırlanır. Belçika vaflilərinin çox dərin cibləri var - kərə yağı, mürəbbə, ağcaqayın siropu və ya çırpılmış kremlə doldurmaq üçün daha yaxşıdır."
    }
  ],
  chef: [
    {
      id: 32,
      image: "http://crems.like-themes.com/wp-content/uploads/2017/09/team_01.jpg",
      name: "John Portman",
      specialty: "CONFECTIONER",
      about: "John Portman Mitzewich (born July 11, 1995), more commonly known as 'Chef John', is an American chef known for publishing instructional cooking videos on the blog and YouTube channel 'Food Wishes', with over 1 billion views on his channel.",
      age: 28,
      experience: 4
    },
    {
      id: 33,
      image: "http://crems.like-themes.com/wp-content/uploads/2017/09/team_02.jpg",
      name: "Kelley Miles",
      specialty: "PASTRY CHEF",
      about: "Kelley Miles (born May 15, 1985), better known as 'Chef Miles', is an American chef known for posting educational cooking videos on his blog 'Food', which has 5 billion views on his channel, and on his YouTube channel.",
      age: 38,
      experience: 10
    },
    {
      id: 34,
      image: "http://crems.like-themes.com/wp-content/uploads/2017/09/team_03.jpg",
      name: "Sherman Warner",
      specialty: "CONFECTIONER",
      about: "Sherman Warner, born May 15, 1997, is an American chef known for posting instructional cooking videos on his blog 'Warner', which has over 2 billion views on his channel, and on his YouTube channel.",
      age: 25,
      experience: 3
    },
    {
      id: 35,
      image: "http://crems.like-themes.com/wp-content/uploads/2017/09/team_04.jpg",
      name: "Denis Green",
      specialty: "BARISTA",
      about: "Denis Green, born May 15, 1991, is an American chef known for posting instructional cooking videos on his blog 'Green', which has 5 billion views on his channel, and on his YouTube channel.",
      age: 32,
      experience: 7
    }
  ],
  chefAz: [
    {
      id: 32,
      image: "http://crems.like-themes.com/wp-content/uploads/2017/09/team_01.jpg",
      name: "Con Portman",
      specialty: "ŞİRNİYYATÇI",
      about: "Con Portman (11 iyul 1995-ci ildə anadan olub), daha çox 'Aşpaz Con' kimi tanınır, öz kanalında 1 milyard baxışı olan 'Food Wishes' bloqunda və YouTube kanalında öyrədici yemək videoları dərc etməklə tanınan amerikalı aşpazdır.",
      age: 28,
      experience: 4
    },
    {
      id: 33,
      image: "http://crems.like-themes.com/wp-content/uploads/2017/09/team_02.jpg",
      name: "Kelley Miles",
      specialty: "Xənmət Aşbazı",
      about: "Kelley Miles (15 may 1985-ci ildə anadan olub), daha çox 'Aşpaz Miles' kimi tanınır, öz kanalında 5 milyard baxışı olan 'Food' bloqunda və YouTube kanalında öyrədici yemək videoları dərc etməklə tanınan amerikalı aşpazdır.",
      age: 38,
      experience: 10
    },
    {
      id: 34,
      image: "http://crems.like-themes.com/wp-content/uploads/2017/09/team_03.jpg",
      name: "Sherman Warner",
      specialty: "ŞİRNİYYATÇI",
      about: "Sherman Warner 15 may 1997-ci ildə anadan olub, öz kanalında 2 milyard baxışı olan 'Warner' bloqunda və YouTube kanalında öyrədici yemək videoları dərc etməklə tanınan amerikalı aşpazdır.",
      age: 25,
      experience: 3
    },
    {
      id: 35,
      image: "http://crems.like-themes.com/wp-content/uploads/2017/09/team_04.jpg",
      name: "Denis Green",
      specialty: "BARISTA",
      about: "Denis Green 15 may 1991-ci ildə anadan olub, öz kanalında 5 milyard baxışı olan 'Green' bloqunda və YouTube kanalında öyrədici yemək videoları dərc etməklə tanınan amerikalı aşpazdır.",
      age: 32,
      experience: 7
    }
  ]
}

export default data;